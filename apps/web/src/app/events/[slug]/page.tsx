...
export default async function EventPage({
  ...
  // Sanity image fields can be `{ _type: "image" }` with no `asset` until a file is chosen.
  // urlFor() requires an asset reference; guard so we never pass an empty image object.
  const heroImage =
    headline?.photo?.asset != null
      ? headline.photo
      : image?.asset != null
        ? image
        : null;
